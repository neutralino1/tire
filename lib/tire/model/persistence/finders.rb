        ...
        module ClassMethods
          ...
          def find *args
            ...
            if args.size > 1
              ...
              case args = args.pop
                when Fixnum, String
                  index.retrieve doc_type, args, :wrapper => self
                when :all, :first
                  send(args)
                else
          ...
          def all
            ...
            s = Tire::Search::Search.new(index.name, :type => doc_type, :wrapper => self).query { all }
            s.version(true).results
          end

          def first
            # TODO: Options like `sort`; Possibly `filters`
            s = Tire::Search::Search.new(index.name, :type => doc_type, :wrapper => self).query { all }.size(1)
